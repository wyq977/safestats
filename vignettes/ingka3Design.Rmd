---
title: "III. Design of experiments: Behaviour of e-variables under alternatives"
author: "Alexander Ly, Rosanne Turner, Udo Boehm, Rianne de Heide, Peter Grunwald"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{III. Design of experiments: Behaviour of e-variables under alternatives}
...
In the previous markdown document we used simulations to illustrate the behaviour of two sequential testing procedures. We saw that if data are generated under the null that the sequential use of classical p-values leads to the incorrect conclusion that the null is false with a chance larger than the tolerable alpha. On the other hand, with e-variables we retain type I error control uniformly across time. 
...
1. In the [first section](#designZ) we clarify how the safe design function determines both the parameter gMom and the planned sample size, if it is given a minimal clinically relevant effect size, and a targeted power/tolerable type II error rate, respectively.  
...
designObj
```
To compute a z-test e-value the function `safeZTest` uses the parameter defined in the designObj, which for the e-value type "mom" is referred to as gMom. This gMom parameter specifies the locations of a so-called non-local moment prior mixture distribution introduced by Johnson and Rossell 2010 in a Bayesian context. Unlike the normal distribution with only one top, this non-local moment distribution is zero at 0, and it has "camel humps" or (two dromedary's humps) away from zero. By selecting gMom equal to 1/2*(meanDiffMin / sigma)^2, that is, `(10/12)^2/2=0.34722` these two humps are located at the minimally clinically relevant effect size `10/12`. 
...
resultObj <- safeZTest(
  x = treatmentGroup, y = controlGroup,
  designObj = designObj
)
resultObj
```
...
```{r, eval=FALSE}
result <- safeZTest(treatmentGroup, controlGroup, designObj = designObj)
plot(result)
...
```{r, eval=FALSE}
designLarger <- designSafeZ(1.2 * designObj$esMin,
  sigma = sigmaTrue,
...
```{r, eval=FALSE}
designSmaller <- designSafeZ(designObj$esMin / 1.2,
  sigma = sigmaTrue,
  parameter = designObj$parameter,
  eType = designObj$eType,
  testType = designObj$testType,
...
```{r, eval=FALSE}
designSafeZ(
  meanDiffMin = designObj$esMin / 1.2, beta = 0.2,
  parameter = designObj$parameter,
...
## Meta-analysis e-values: Combining the evidence across studies by multiplication 
...
firstPassageTimes <- designSmaller$bootObjBeta$data

followUpStudy <- sampleStoppingTimesSafeZ(
  meanDiffTrue = designObj$esMin / 1.2,
  sigma = sigmaTrue,
  parameter = designObj$parameter,
  eType = designObj$eType,
  testType = designObj$testType,
